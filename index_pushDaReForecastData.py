...
logger.info(f"------Insertion started of DayAhead solar&wind forecast file b/w {dt.datetime.strftime(startDate, '%Y-%m-%d')}To{dt.datetime.strftime(endDate, '%Y-%m-%d')}--------- ")
...
# 20th re da file contain DA RE forecast of 20
while currdate<=endDate:
    ...
    for state in stateList:
        ...
        for singleReFile in reForecastFiles:
            ...
            if (isFoundCurrDayCurStateSolarFile==False) and (currdateStrYYYYMMDD in singleReFile) and (stateName in singleReFile) and ('da' in singleReFile) and ('Solar' in singleReFile) and (singleReFile.endswith(('.csv'))):               
                solarForecastFileNamePath = reForecastFolderPath + singleReFile
                stateSolarForecastDf = pd.read_csv(solarForecastFileNamePath, usecols= range(1,3),skipfooter=1, engine='python')
            ...
            if (isFoundCurrDayCurStateWindFile==False) and (currdateStrYYYYMMDD in singleReFile) and (stateName in singleReFile) and ('da' in singleReFile) and ('Wind' in singleReFile) and (singleReFile.endswith(('.csv'))):   
                ...
                stateWindForecastDf = pd.read_csv(windForecastFileNamePath, usecols= range(1,3), skipfooter=1, engine='python')
