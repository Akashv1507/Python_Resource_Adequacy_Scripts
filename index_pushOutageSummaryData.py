...
outageSummaryService = OutageSummaryService(appConfig["postgresqlHost"], appConfig["postgresqlPort"], appConfig["postgresqlDb"], appConfig["postgresqlUser"], appConfig["postgresqlPass"], appConfig["con_string_outage_db"])
...
while currdatetime<=endDateTime: 
    ...
    for index, row in capacitySummaryDf.iterrows():
        stateName= row['STATE_NAME']
        classification = row['CLASSIFICATION']
        stationType = row['STATION_TYPE']
        outageCapacity = row['INSTALLED_CAPACITY']
        installedCapacity = 0
        intradayDcofBlk = 0
        daDcOfBlk = 0
        normDc = 0
        if classification == 'STATE_OWNED' and stationType in validStationTypesList:
            # getting total installed capacity of particular state,classification, stationType
            filteredStateInstCapacityDf = stateInstCapacityMetaDataDf[(stateInstCapacityMetaDataDf['STATE_NAME']==stateName) & (stateInstCapacityMetaDataDf['STATION_TYPE']== stationType) & (stateInstCapacityMetaDataDf['CLASSIFICATION']== classification)]
            if not filteredStateInstCapacityDf.empty:
                installedCapacity= filteredStateInstCapacityDf['INSTALLED_CAPACITY'].iloc[0]
