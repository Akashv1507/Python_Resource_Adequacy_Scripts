...
logger.info("----------Chattisgarh Solar Forecast Insertion Started--------------")

def generateSolarForecastValList(dateKey:dt.datetime):
    
    solarForecastValList=[]
    dateStr = dateKey.strftime('%d_%m_%Y')
    # CHHATTISGARH_18_04_2024
    chattFilePath =   f'{finalDumpFolderPath}CHHATTISGARH_{dateStr}.xlsx' 
    try:
        chattSolarDf = pd.read_excel(chattFilePath, skiprows=3, usecols=[13, 17], nrows=48)
        chattSolarDf.columns= ['val1', 'val2']     
        for ind in chattSolarDf.index:
           solarForecastValList.append(chattSolarDf['val1'][ind])
        for ind in chattSolarDf.index:
           solarForecastValList.append(chattSolarDf['val2'][ind])
        return solarForecastValList
    except Exception as err:
        logger.error(f"Error while reading chatt file for {dateStr} with {err}")
...
solarForecastValDf['Entity_Tag']="WRLDCMP.SCADA1.A0046945"
